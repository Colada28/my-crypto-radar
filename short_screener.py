...
# ФИЛЬТРЫ
MIN_VOLUME_24H = 1000000  # от $1,000,000 объема
MIN_LIQ_AMOUNT = 3000     # от $3,000 за ордер

bot = telebot.TeleBot(TELEGRAM_TOKEN)

def get_active_futures():
    """Получает активные пары с Bybit"""
...
def send_alert(symbol, side, amount_usd, price, vol24h):
    ...
    try:
        ...
        print(f"🔥 Сигнал по {symbol} отправлен!", flush=True)
    except Exception as e:
        print(f"Ошибка ТГ: {e}", flush=True)

if __name__ == "__main__":
    print("=== Скринер Bybit запущен в чистом режиме ===", flush=True)
